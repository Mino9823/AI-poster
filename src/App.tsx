...
const QuotePosterPage: React.FC = () => {       
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('background_style',"poster");
    ...
    try {
      ...
      alert(`포스터 생성 성공! 결과 확인: ${JSON.stringify(result)}`);
      ...
      {/* 2. import 한 변수명(iconSrc)을 src에 사용 */}
        ...
        <FormCard onSubmit={handleSubmit}>
          <InputWrapper>
            ...
            <SubmitButton type="submit">명언 생성</SubmitButton>
          </InputWrapper>
...
const PresetChip = styled.button`
  ...
  padding: 8px 18px;
  font-size: 14px;
  background: linear-gradient(135deg, #f1f5ff, #f9fdff);
  color: #4e5e8b;
  box-shadow: 0 10px 22px rgba(124, 144, 255, 0.18);
  cursor: pointer;
  white-space: nowrap;
  ...
  &:hover {
    transform: translateY(-1px);
    box-shadow: 0 14px 30px rgba(124, 144, 255, 0.25);
    background: linear-gradient(135deg, #e4edff, #f4fbff);
  }

  &:active {
    transform: translateY(0);
    box-shadow: 0 8px 16px rgba(124, 144, 255, 0.2);
  }
`;
